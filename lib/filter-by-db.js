...
module.exports = function (socketID) {
    return function localFilter(profiles) {
        ...
        return Promise.all(profiles.map(profile => {
            ...
            // Validate profiles
            .then(steamProfiles => steamProfiles.map((steamProfile, index) => {
                if (Object.keys(steamProfile).length === 0) throw {
                ...
                if (steamProfile.response !== undefined &&
                    ...
                    throw new Error('Invalid profile: ' + profiles[index]);

                if (steamProfile.gamesList !== undefined &&
                    steamProfile.gamesList.error !== undefined)
                    throw new Error('Private profile: ' + profiles[index]);

                if (steamProfile.gamesList === undefined)
                    throw new Error('Games not found for ' + profiles[index]);

                return steamProfile.gamesList.games.game.map(
