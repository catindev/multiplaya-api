...
class Multiplaya extends EventEmitter {
    ...
    getProfileURL(dude) {
        ...
        return `http://steamcommunity.com/${dudeType}/${dudeName}/games?tab=all&xml=1`;
    }
    ...
    getGames(dudes) {
        ...
        this.dudes.forEach((dude, index) => {
            ...
            request(this.getProfileURL(dude), (error, response, body) => {
                const responseData = JSON.parse(parser.toJson(body));
                this.error = this.isProfileError(dude, responseData);
